...
export async function sendTicketEmail({ to, buyerName, tickets, event }) {
  ...
  const ticketRows = tickets
    .map((ticket, i) => {
      ...
      return `
        <div style="background:#0A0712;border:1px solid #332942;border-radius:12px;padding:14px;margin-top:${i === 0 ? "20" : "10"}px;">
          <p style="color:#9B93A8;font-size:12px;margin:0 0 8px;">Entrada ${i + 1} de ${count} · <span style="font-family:monospace;color:#E91E8C;">${ticket.code}</span></p>
          <a href="${viewUrl}" style="display:block;text-align:center;background:#E91E8C;color:#0A0712;font-weight:bold;font-size:14px;text-decoration:none;padding:12px;border-radius:999px;">
  ...
  const html = `
    <div style="background:#0A0712;padding:32px 16px;font-family:Helvetica,Arial,sans-serif;">
      <div style="max-width:480px;margin:0 auto;background:#17111F;border-radius:16px;overflow:hidden;border:1px solid #332942;">
        <div style="padding:28px 28px 20px;text-align:center;">
          ...
          <p style="color:#9B93A8;font-size:14px;margin:0;">Hola ${buyerName || ""}, gracias por tu compra.</p>
        </div>
        <div style="padding:0 28px 24px;">
          <div style="background:#0A0712;border:1px solid #332942;border-radius:12px;padding:16px;">
            ...
            <p style="color:#9B93A8;font-size:13px;margin:0;">${event.dateLabel}</p>
            <p style="color:#9B93A8;font-size:13px;margin:0 0 12px;">${event.location}</p>
            <p style="color:#9B93A8;font-size:13px;margin:0;">${count} entrada${count > 1 ? "s" : ""} · ${euros(totalCents)}€</p>
          </div>

          ${
            count > 1
              ? `<p style="color:#9B93A8;font-size:12px;text-align:center;margin-top:16px;">Cada entrada tiene su propio código — si vais varios, cada uno enseña la suya en la puerta.</p>`
              : ""
          }
...
export async function sendMerchandiseEmail({ to, buyerName, purchases, merchandise }) {
  ...
  const purchaseRows = purchases
    .map((purchase, i) => {
      ...
      return `
        <div style="background:#0A0712;border:1px solid #332942;border-radius:12px;padding:14px;margin-top:${i === 0 ? "20" : "10"}px;">
          <p style="color:#9B93A8;font-size:12px;margin:0 0 8px;">Unidad ${i + 1} de ${count} · <span style="font-family:monospace;color:#E91E8C;">${purchase.code}</span></p>
          <a href="${viewUrl}" style="display:block;text-align:center;background:#E91E8C;color:#0A0712;font-weight:bold;font-size:14px;text-decoration:none;padding:12px;border-radius:999px;">
    ...
    <div style="background:#0A0712;padding:32px 16px;font-family:Helvetica,Arial,sans-serif;">
      <div style="max-width:480px;margin:0 auto;background:#17111F;border-radius:16px;overflow:hidden;border:1px solid #332942;">
        <div style="padding:28px 28px 20px;text-align:center;">
          ...
          <p style="color:#9B93A8;font-size:14px;margin:0;">Hola ${buyerName || ""}, esto es lo que has comprado.</p>
        </div>
        <div style="padding:0 28px 24px;">
          <div style="background:#0A0712;border:1px solid #332942;border-radius:12px;padding:16px;">
            <p style="color:#F5F1E8;font-size:17px;font-weight:bold;margin:0 0 8px;">${merchandise.name}</p>
            <p style="color:#9B93A8;font-size:13px;margin:0;">${count} unidad${count > 1 ? "es" : ""} · ${euros(totalCents)}€</p>
          </div>

          ${
            count > 1
              ? `<p style="color:#9B93A8;font-size:12px;text-align:center;margin-top:16px;">Cada unidad tiene su propio código, para poder recogerlas por separado si hace falta.</p>`
              : ""
          }
...
export async function sendPasswordResetEmail({ to, name, code }) {
  ...
  const html = `
    <div style="background:#0A0712;padding:32px 16px;font-family:Helvetica,Arial,sans-serif;">
      <div style="max-width:420px;margin:0 auto;background:#17111F;border-radius:16px;overflow:hidden;border:1px solid #332942;">
        <div style="padding:28px;text-align:center;">
          <h1 style="color:#F5F1E8;font-size:20px;margin:0 0 8px;">Recupera tu contraseña</h1>
          <p style="color:#9B93A8;font-size:14px;margin:0 0 24px;">Hola ${name || ""}, usa este código en la app para elegir una contraseña nueva. Caduca en 15 minutos.</p>
          <div style="background:#0A0712;border:1px solid #332942;border-radius:12px;padding:20px;">
            <p style="color:#E91E8C;font-family:monospace;font-size:32px;letter-spacing:0.15em;margin:0;">${code}</p>
          </div>
          <p style="color:#9B93A8;font-size:12px;margin-top:20px;">
